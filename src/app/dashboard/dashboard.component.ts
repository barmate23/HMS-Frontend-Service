...
export class DashboardComponent {
  ...
  readonly totalMonthlyRevenue = computed(() => Number(this.dashboardData()?.revenueAndBookings?.totalRevenue ?? this.dashboardData()?.summary?.fyBookingRevenue ?? this.monthMetrics().reduce((sum, item) => sum + item.revenue, 0)));
  readonly totalBookings = computed(() => Number(this.dashboardData()?.revenueAndBookings?.totalBookings ?? this.monthMetrics().reduce((sum, item) => sum + item.bookings, 0)));
  ...
  readonly posOrderCount = computed(() => Number(this.dashboardData()?.summary?.posOrders ?? this.fyPosOrders().length));
  readonly posOrderValue = computed(() => Number(this.dashboardData()?.posPerformance?.orderValue ?? this.fyPosOrders().reduce((sum, order) => sum + this.orderValue(order), 0)));
  ...
  readonly posMenuItemCount = computed(() => Number(this.dashboardData()?.posPerformance?.menuItemsCount ?? this.posMenuItems().length));
    ...
    this.http.get<StandardResponse<DashboardDataDto>>(this.dashboardUrl).subscribe({
      next: response => {
        if (response?.success && response.data) {
          ...
          this.useSampleDashboardData();
        }
        this.isLoadingRevenue.set(false);
      },
      error: error => {
        this.useSampleDashboardData(error?.error?.message || error?.message || 'Unable to load dashboard data.');
        this.isLoadingRevenue.set(false);
      }
    });
  }
  ...
  private useSampleDashboardData(message = 'Showing sample booking data.'): void {
  ...
  private mapPosItemStat(item: PosItemStatDto): PosItemMetric {
    ...
    return {
      ...
      subcategory: 'Item',
      imageUrl: item.imageUrl || this.itemImageUrl(item.itemName || ''),
      price,
      qty,
      value,
      monthVariation
  ...
  private monthVariationFromTrend(trend: MonthlyStatDto[]): PosItemMetric['monthVariation'] {
    ...
    const rows = months.map(month => ({
      label: month.label.toLowerCase(),
      qty: Number(byLabel.get(month.label.toLowerCase())?.bookings || 0),
      height: 8
    }));
